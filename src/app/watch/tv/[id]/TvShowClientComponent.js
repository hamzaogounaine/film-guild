...
export const TVShowClientComponent = ({ initialTvShowData, params, servers }) => {
    ...
    useEffect(() => {
        if (id) {
            ...
            let seasonToSet = initialSeason;
            let episodeToSet = initialEpisode;

            if (savedProgress) {
                try {
                    const parsedData = JSON.parse(savedProgress);
                    if (parsedData.season && parsedData.episode) {
                        seasonToSet = parsedData.season;
                        episodeToSet = parsedData.episode;
                    }
                } catch (error) {
                    console.error("Error parsing saved progress:", error);
                }
            } else {
                // If no localStorage, set to the first "regular" season (season_number > 0)
                const firstRegularSeason = initialTvShowData.seasons?.find((season) => season.season_number > 0);
                if (firstRegularSeason) {
                    seasonToSet = firstRegularSeason.season_number;
                }
            }
    ...
    const getCurrentEpisode = () => episodes.find((ep) => ep.episode_number === selectedEpisode);
    const getCurrentSeason = () => tvShow.seasons?.find((season) => season.season_number === selectedSeason);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br bg-gray-900 text-white">
            ...
            {/* Content Section (Episode List, Servers, Season Info) */}
            <div className="relative z-10 px-4 md:px-16 pb-16">
                <div className="max-w-7xl mx-auto">
                    <div className="grid lg:grid-cols-3 gap-8">
                        {/* Episode Selection & Server Status */}
